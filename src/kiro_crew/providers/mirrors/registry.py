...
PROJECTIONS: dict[str, McpProjection] = {
    ...
    ACP_BACKEND_PI: McpProjection(
        ...
        "session/new array is this backend's whole MCP channel",
        # PER-CALL, on codex's two-form story, with one proven channel: a narrowed
        # third-party server is WITHHELD whole (pi.py withholds what
        # ``session_mcp_restricted_servers`` names, exactly like codex), because a
        # third-party call under pi-acp never reliably asks. Crew's own CONTROL
        # PLANE stays mounted, and the allowance rides pi's canonical
        # ``mcp__<server>__<tool>`` spelling: the adapter requests permission per
        # MCP call with that title, so ``AcpClient._deny_spec_disabled_tool``
        # matches the pair the mirror's ``denied_tools`` hands over and refuses the
        # call itself (pi.py, ``client.py``), where opencode has no identity to
        # match and must withhold the server whole.
        per_tool_deny=PerToolDeny.PER_CALL,
    ),
}
