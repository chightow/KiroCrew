...
and Pi stops being the guaranteed path.

| Id | Guarantees | Pinned by | Constrains |
|---|---|---|---|
| H1 | `agent.acp_backend` defaults to `ACP_BACKEND_PI`, and `ACP_BACKEND_PI` is in `selectable_backends()` unconditionally. An operator who configures nothing, and an operator whose configuration is unusable, both get the Pi harness. | `test_harness_parity.py::test_pi_is_the_default_backend`, `::test_pi_is_always_selectable` | `config/loader.py` (`AgentConfig.acp_backend`), `acp_backends.py` (`BASELINE_SELECTABLE_BACKENDS`) |
